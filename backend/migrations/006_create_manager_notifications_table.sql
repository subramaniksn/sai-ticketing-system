...
CREATE TABLE IF NOT EXISTS manager_notifications (
    ...
    manager_id INTEGER NOT NULL REFERENCES users(id) ON DELETE SET NULL, -- Manager who sent notification
    dispatcher_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE, -- Recipient dispatcher
...
BEGIN
    IF NEW.notification_status != OLD.notification_status THEN
        ...
        -- Auto-update viewed_at if status changed to viewed or acknowledged
        ...
        -- Auto-update acknowledged_at if status changed to acknowledged
