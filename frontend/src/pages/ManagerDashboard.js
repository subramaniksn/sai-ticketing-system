...
{/* 🔔 NOTIFY DISPATCHER BUTTON */}
<div style={{padding:'15px', background:'#fff3cd', borderRadius:'8px', margin:'15px', border:'1px solid #ffc107'}}>
  <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
    <h3 style={{margin:0}}>📞 Notify Dispatcher</h3>
    <button
      onClick={() => setShowNotifyForm(!showNotifyForm)}
      style={{background:'#f39c12', color:'white', border:'none', padding:'8px 16px', borderRadius:'6px', cursor:'pointer', fontWeight:'bold'}}>
      {showNotifyForm ? '✕ Close' : '+ New Notification'}
    </button>
  </div>
  ...
  {showNotifyForm && (
    <div style={{marginTop:'15px', display:'grid', gap:'10px'}}>
      ...
      />
      <textarea
        placeholder="Issue Details * (describe the problem)"
